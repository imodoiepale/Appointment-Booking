...
-- Rename meetings table (if it exists and bcl_meetings_meetings_duplicate doesn't exist)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'meetings') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'bcl_meetings_meetings_duplicate') THEN
        -- Rename the table
        ALTER TABLE meetings RENAME TO bcl_meetings_meetings_duplicate;
        RAISE NOTICE 'Table "meetings" renamed to "bcl_meetings_meetings_duplicate"';
    ELSE
        RAISE NOTICE 'Table rename skipped - either "meetings" doesn''t exist or "bcl_meetings_meetings_duplicate" already exists';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS bcl_meetings_meetings_duplicate (
...
CREATE TABLE IF NOT EXISTS bcl_meetings_reminder_logs (
    ...
    meeting_id BIGINT REFERENCES bcl_meetings_meetings_duplicate(id_main) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_bcl_meetings_meetings_start_time ON bcl_meetings_meetings_duplicate(meeting_start_time);
CREATE INDEX IF NOT EXISTS idx_bcl_meetings_meetings_date ON bcl_meetings_meetings_duplicate(meeting_date);
...
SELECT 
    'bcl_meetings_meetings_duplicate' as table_name,
